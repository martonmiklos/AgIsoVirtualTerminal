...
{
	return ((get_data_mask_area_size_y_pixels() - SoftKeyMaskDimensions::padding) / (get_soft_key_descriptor_y_pixel_width() + SoftKeyMaskDimensions::padding));
}

uint8_t ServerMainComponent::get_physical_soft_key_rows() const
{
	return ceil(get_data_mask_area_size_y_pixels() / get_physical_soft_key_columns());
}
...
void ServerMainComponent::resized()
{
	...
	softKeyMaskRenderer.setBounds(100 + get_data_mask_area_size_x_pixels(),
	                              lMenuBarHeight + 4,
								  2 * SoftKeyMaskDimensions::padding + get_physical_soft_key_columns() * (SoftKeyMaskDimensions::padding + get_soft_key_descriptor_y_pixel_width()),
...
bool ServerMainComponent::perform(const InvocationInfo &info)
{
	...
	switch (info.commandID)
	{
		...
		case static_cast<int>(CommandIDs::ConfigureReportedHardware):
		{
			...
			popupMenu->addTextEditor("Number of Physical Soft Key columns", String(get_number_of_physical_soft_keys()), "Number of Physical Soft Key columns (min 1)");
			popupMenu->addTextEditor("Number of Physical Soft Key rows", String(get_number_of_physical_soft_keys()), "Number of Physical Soft Key rows (min 1)");
...
void ServerMainComponent::LanguageCommandConfigClosed::operator()(int result) const noexcept
{
	switch (result)
	{
		...
		case 3: // Save Reported Hardware
		{
			...
			mParent.softKeyMaskDimensions.keyCount = mParent.popupMenu->getTextEditorContents("Number of Physical Soft Keys").getIntValue();
			...
			if (nullptr != xml)
			{
				...
				while (child.isValid())
				{
					...
					else if (Identifier("Hardware") == child.getType())
					{
						...
						{
							softKeyMaskDimensions.keyCount = static_cast<std::uint16_t>(static_cast<int>(child.getProperty("PhysicalSoftkeys")));
						}
						...
						if (!child.getProperty("SoftkeyColumnRowCount").isVoid())
						{
							softKeyMaskDimensions.rowCount = static_cast<std::uint16_t>(static_cast<int>(child.getProperty("SoftkeyColumnRowCount")));
						}
						if (!child.getProperty("DataMaskRenderAreaSize").isVoid())
						{
							...
							softKeyMaskRenderer.setSize(2 * SoftKeyMaskDimensions::padding + get_physical_soft_key_columns() * (SoftKeyMaskDimensions::padding + get_soft_key_descriptor_y_pixel_width()),
...
void ServerMainComponent::save_settings()
{
	...
	if (lCanSaveSettings)
	{
		...
		hardwareSettings.setProperty("PhysicalSoftkeys", softKeyMaskDimensions.keyCount, nullptr);
